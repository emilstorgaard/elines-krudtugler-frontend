<script lang="ts">
	import type { ContactPageProperties } from '$lib/types/contactPage';

	let { contact }: { contact: ContactPageProperties } = $props();
	...
	const directionsButtonUrl = $derived(
		fullAddress
...
<div class="px-4 sm:px-6">
	...
	<div class="my-8 grid items-stretch gap-5 sm:my-14 sm:gap-8 lg:grid-cols-5">
		...
		<div class="flex flex-col gap-3 sm:gap-5 lg:col-span-2" class:lg:col-span-5={!mapsEmbedUrl}>
			{#if p.addressCardLabel || p.directionsButtonText}
				<div
					class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:rounded-3xl sm:p-6"
				>
					...
					{#if p.directionsButtonText && directionsButtonUrl}
						<a
