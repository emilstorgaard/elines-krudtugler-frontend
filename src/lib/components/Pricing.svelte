<script lang="ts">
    ...
    const subTiles = $derived(
        p.pricingSubTiles?.items?.map((item) => item.content.properties) ?? []
    );

    // Hovedpris-kort vises kun hvis der er noget data
...
{#if hasContent}
    <section id="priser" class="relative overflow-hidden px-4 py-8">
        <div class="relative mx-auto max-w-3xl">
            ...
            {#if p.pricingIntro}
                <p
                    class="mx-auto mb-12 max-w-2xl text-center text-lg leading-relaxed text-gray-500 whitespace-pre-line"
                >
                    {p.pricingIntro}
                </p>
            {/if}
                    ...
                    {#each infoTiles as tile}
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p class="text-sm leading-relaxed text-gray-500 whitespace-pre-line">
                                    {tile.description}
                                </p>
                            </div>
                        </div>
