<script lang="ts">
	...
	function logoSrc(url: string) {
		return mediaSrc(url, 'footerLogo');
	}

	function logoSrcset(url: string) {
		return mediaSrcset(url, 'footerLogo');
	}
</script>
	...
	<div class="mx-auto max-w-6xl">
		<div class="flex flex-col items-center justify-between gap-5 sm:flex-row sm:gap-6">
			<a href="/" class="group flex items-center gap-3">
				{#if logo}
					<img
						src={logoSrc(logo.url)}
						srcset={logoSrcset(logo.url)}
						sizes="64px"
						alt={s.siteName}
						width="64"
						height="64"
					...
					/>
				{/if}
				<span class="text-base font-bold tracking-tight sm:text-lg">{s.siteName}</span>
			</a>
