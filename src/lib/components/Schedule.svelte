<script lang="ts">
	import type { WeekdayPageProperties } from '$lib/types/weekdayPage';

	let { schedule }: { schedule: WeekdayPageProperties } = $props();
	...
	{#if p.activitiesHeading || p.activitiesContent}
		...
		>
			{#if p.activitiesHeading}
				<div
					class="border-b border-brand-100/70 bg-linear-to-r from-brand-50 to-white px-5 py-4 sm:px-6 sm:py-5"
				>
					<h3
						class="text-center text-base font-bold tracking-tight text-gray-800 sm:text-lg md:text-xl"
					>
						{p.activitiesHeading}
					</h3>
				</div>
			...
			<div class="space-y-5 px-5 py-5 text-gray-700 sm:space-y-6 sm:px-6 sm:py-6 md:px-8">
				...
				{#if p.focusHeading || p.focusAreas}
					<div
						class="rounded-xl bg-brand-50/70 p-3.5 ring-1 ring-brand-100/70 sm:rounded-2xl sm:p-4"
					>
						{#if p.focusHeading}
