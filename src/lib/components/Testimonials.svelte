...
<section id="anmeldelser" class="relative overflow-hidden bg-white px-4 py-24">
	...
	<div class="absolute -bottom-24 left-0 h-80 w-80 rounded-full bg-brand-50/60 blur-3xl"></div>
	...
	<div class="relative mx-auto max-w-6xl">
		...
		<div class="hidden gap-8 md:grid md:grid-cols-3">
			{#each visible as testimonial (testimonial.name)}
				<div
					class="group relative flex flex-col rounded-3xl border border-gray-100 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand-200 hover:shadow-xl"
				>
					<div
						class="absolute top-6 right-8 font-serif text-6xl leading-none text-brand-100 select-none"
					>
						"
					</div>
		...
		{#if totalPages > 1}
			<div class="mt-10 hidden items-center justify-center gap-4 md:flex">
				<button
					onclick={prev}
					class="flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-white shadow-sm transition-all hover:border-brand-300 hover:bg-brand-50"
					aria-label="Forrige"
				>
		...
		<!-- Summary bar -->
		<div
			class="mt-16 flex flex-col items-center justify-center gap-6 rounded-3xl bg-brand-50 px-8 py-8 sm:flex-row"
		>
			...
			<div class="hidden h-12 w-px bg-brand-200 sm:block"></div>
			<div class="text-center">
