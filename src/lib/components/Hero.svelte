<script lang="ts">
    ...
    // Hero-billeder fra Umbraco
    const heroImages = $derived(
        p.heroImages?.map((img) => getMediaUrl(img.url)) ?? []
    );

    // Nav-links fra Settings (Block List)
    const heroNavLinks = $derived(
