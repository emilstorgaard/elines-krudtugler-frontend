	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="flex h-20 items-center justify-between">
			...
			>
				...
				<div class="text-left">
					<span class="block text-lg font-bold leading-tight text-gray-800">Børnepasning</span>
					<span class="block text-xs font-medium tracking-wide text-green-600">Eline Storgaard Andersen</span>
				</div>
			</button>
			...
			<nav class="hidden items-center gap-1 md:flex">
				{#each navItems as item (item.href)}
					...
					>
						{item.label}
						<span class="absolute -bottom-0.5 left-1/2 h-0.5 w-0 -translate-x-1/2 rounded-full bg-green-500 transition-all duration-300 group-hover:w-4/5"></span>
					</button>
				{/each}

				<button
					on:click={() => handleNavClick('#kontakt')}
