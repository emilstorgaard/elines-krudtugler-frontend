<script lang="ts">
    ...
    // Vis ledige pladser panel kun hvis toggle er TIL
    const showAvailability = $derived(p.availabilityEnabled === true);

    // Generér href fra phone (fjern mellemrum og special chars)
...
<div class="overflow-hidden my-14 rounded-3xl border border-gray-100 bg-white shadow-xl">
    <div class="grid" class:md:grid-cols-2={showAvailability}>
        ...
        <!-- Availability panel - vises kun hvis toggle er TIL -->
        {#if showAvailability}
            <div
            ...
            >
                <div>
                    ...
                    {#if p.availabilityDescription}
                        <p class="mb-8 leading-relaxed text-white/85 whitespace-pre-line">
                    ...
                    {#if p.availabilityNote}
                        <div class="rounded-2xl bg-white/15 p-4 backdrop-blur-sm">
                            <p class="text-sm leading-relaxed text-white/90 whitespace-pre-line">
                                {p.availabilityNote}
                            </p>
                        </div>
                    {/if}
